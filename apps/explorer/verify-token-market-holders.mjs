...
console.log('5. wiring presence checks')
{
  ...
  ok(holdersLib.includes('getDataProvider'), '5.holders lib resolves the data provider (vendor behind the adapter)')
  ok(holdersLib.includes("source: 'moralis'"), "5.holders lib labels provider results source='moralis' (UI contract)")
  ok(!/goldrush/i.test(holdersLib), '5.no goldrush refs left in holders lib')

  const moralisLib = readSrc('apps/explorer/lib/providers/moralis.ts')
  ok(moralisLib.includes('async getTokenHolders('), '5.provider adapter exposes getTokenHolders')
  ok(moralisLib.includes('async getTokenHolderCount('), '5.provider adapter exposes getTokenHolderCount')
  ok(moralisLib.includes('MORALIS_HOLDERS_DAILY_MAX'), '5.moralis caps env-overridable (per-bucket)')
